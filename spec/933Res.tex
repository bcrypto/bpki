...
\item
\sstr{termsOfServiceAgreed} (необязательное, логическое значение)~--- 
...
по которому размещены адреса объектов авторизации. Объекты можно получить 
...
сопроводив его значением \code{true} в поле \sstr{wildcard}.
...
может измениться на \sstr{expired} по истечении срока действия объекта,
...
с \sstr{valid} на \sstr{expired}, \sstr{revoked} или \sstr{deactivated}).
