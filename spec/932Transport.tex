...
или СТБ~34.101.90 (TLS версии 1.3). В этом протоколе сервер ACME выступает в роли 
...
(см.~\cite{HTTP}). В заголовке клиенту следует указывать атрибуты программного 
обеспечения ACME, а также название и версию программного обеспечения HTTP.
...
%
Объект JSON, в кодовом представлении двоичных полей которого  
...
%
Типы ошибок в поле \sstr{type} должны начинаться с префикса 
...
указан в \sstr{url}. При обнаружении неточностей сервер должен отклонить запрос.
...
об ошибке.
%
Сервер должен определять область действия синхропосылок достаточно широко, 
...
уровня нагрузки, сервер в своих ответах должен указывать тип ошибки 
\sstr{rateLimited}.
%
