...
static FILE *output = 0;			/* Output stream */
...
static void displayString( FILE *inFile, long length, int level,
	...
	for( i = 0; i < noBytes; i++ )
		...
		if( strOption == STR_UTF8 && ( ch & 0x80 ) )
			...
			if( ( ch & 0xE0 ) == 0xC0 )		/* 2-byte character? */
			{
				wCh = (wchar_t)secondCh, wCh <<= 8, wCh |= (wchar_t)ch;
				i++;
				fPos += 2;
			}
			...
			{
				const int forthCh = getc( inFile );
				wCh = '.';
				i++;
				fPos += 4;
			}

			if( !displayUTF8( wCh, level ) )
				printString( level, "%c", '.' );
			lineLength++;
			continue;
			}
...
static void usageExit( void )
	...
	puts( "Usage: dumpasn1 [-acdefghilmoprstuvwxz] <file> <out>" );
	puts( "  Input options:" );
		...
		while( *argPtr )
			...
			switch( toupper( byteToInt( *argPtr ) ) )
				...
				case 'S':
					doCheckOnly = TRUE;
#if defined( __WIN32__ )
					( void ) freopen( "nul", "w", output );
#elif defined( __UNIX__ )
					( void ) freopen( "/dev/null", "w", output );
#else
					fclose( output );
#endif
					break;
