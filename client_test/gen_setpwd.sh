...
openssl asn1parse -genstr "UTF8:$newpwd" -out out/$1/setpwd.der > /dev/null

bash dump.sh out/$1/setpwd.der
...
openssl cms -sign -signer answers/$1/cert \
  ...
  -out out/$1/signed_setpwd.der -outform der -nodetach -nosmimecap
...
openssl cms -encrypt -binary -in out/$1/signed_setpwd.der -inform der \
  -recip out/ca1/cert -belt-cfb256 \
  -out out/$1/enveloped_signed_setpwd.der -outform der
...
openssl dgst -belt-hash out/$1/enveloped_signed_setpwd.der \
  -hex > out/$1/setpwd_reqid.txt 2> /dev/null
